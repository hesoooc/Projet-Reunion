...
class Messages
  ...
  def self.stringToKey(str)
    if str && str[/[\r\n\t\1]|^\s+|\s+$|\s{2,}/] # see if any processing is needed
      key = str.clone
      key.gsub!(/^\s+/, "") # trim leading whitespace
      key.gsub!(/\s+$/, "") # trim trailing whitespace
      key.gsub!(/\s{2,}/, " ") # whitespace runs are replaced with a single space
      return key
    end
    return str
  end
