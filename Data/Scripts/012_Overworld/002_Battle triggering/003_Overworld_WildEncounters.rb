...
class PokemonEncounters
  ...
  def encounter_type
    ...
    if $PokemonGlobal.surfing
      ...
      if $game_map.terrain_tag($game_player.x, $game_player.y).id == :PurpleGrass 
        ret = find_valid_encounter_type_for_time(:PurpleLand, time)
      end
      if has_land_encounters? && $game_map.terrain_tag($game_player.x, $game_player.y).land_wild_encounters && !ret
