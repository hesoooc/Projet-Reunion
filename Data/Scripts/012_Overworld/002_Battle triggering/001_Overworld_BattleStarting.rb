...
module BattleCreationHelperMethods
  ...
  def set_up_player_trainers(foe_party)
    ...
    if partner_can_participate?(foe_party)
      ally = NPCTrainer.new(pbGetMessageFromHash(MessageTypes::TrainerNames, $PokemonGlobal.partner[1]), $PokemonGlobal.partner[0])
...
class WildBattle
  ...
  def self.start(*args, can_override: false)
    ...
    if foe_party.length == 1 && can_override
      handled = [nil]
      EventHandlers.trigger(:on_calling_wild_battle, foe_party[0].species_data.id, foe_party[0].level, handled)
    ...
    if foe_party.length == 1 && can_override
      EventHandlers.trigger(:on_wild_battle_end, foe_party[0].species_data.id, foe_party[0].level, outcome)
    end
...
PICKUP_COMMON_ITEMS = [
  ...
  :MAXREPEL,      # Levels 1-10, 11-20, 21-30, 31-40, 41-50, 51-60
